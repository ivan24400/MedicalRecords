...
contract medicalRecords is ERC20Interface {
    ...
    mapping(uint => Doctor) public doctorInfo;
    mapping(uint => Patient) public patientInfo;
    ...
    function setDoctorDetails(uint _dId, string memory _dName, string memory _dEmail, uint _dContNum, string memory _dEducation, string memory _dAdd) onlyHospitalOwner public {   //Set the details of doctors
        require(isRecordExist[_dId] != true, "The Doctor details are already exist");
    ...
    function setPatientDetails(uint _pId, string memory _pName, string memory _pEmail, uint _pContNum, string memory _padd, uint _consultedDoctor)  public returns(bool) {   //This function is used to set the patient details.
        require(isRecordExist[_pId] != true, "The Patient record is already exist");
    ...
    function getDoctorDetails(uint _dId) public view returns(string memory, string memory, uint, string memory, string memory) {    //Get doctor details
        require(isRecordExist[_dId] == true, "The Doctor details does NOT exists");
    ...
    function getPatientInfo(uint _pId) public view returns(string memory, string memory, uint, string memory, uint) {    //Get Patient details.
        require(isRecordExist[_pId] == true, "The Patient details does NOT exists");
